frap = function(Z, nmcmc, burnin, thin, a_tau, b_tau, n_cores, prop_sd, prop_sd_H, nugget, smooth.posterior = F)
{
  ...
  for(ii in 1:nmcmc)
  {
    ...
    S = 0.5*sum(diag(t(what- G)%*%Sigma_H_inv%*%(what - G))) + (0.5/tau^2)*(t(g)%*%cov_mat_diff_inv%*%g)
